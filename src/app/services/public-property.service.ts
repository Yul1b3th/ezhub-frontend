...
export class PublicPropertyService {
  ...
  private fetchProperties(): Observable<Property[]> {
    ...
    return this.http.get<Property[]>(`${this.baseUrl}/public-properties`).pipe(
  ...
  private filterAvailableProperties(properties: Property[]): Property[] {
    return properties.filter(
      (property) => property.deletedAt == null && property.is_available
    );
  }
  ...
  getPropertyById(id: number): Observable<Property | null> {
    return this.http
      ...
      .pipe(
        catchError((error) => {
          this.notificationService.showNotification(
            `Error fetching property by ID ${id}: ${error.message}`,
            'error'
          );
